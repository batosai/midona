...
<template>
    ...
    <div class="w-full h-full">
      <Menubar class="sticky top-0 border-0 rounded-none z-100 bg-surface-100 dark:bg-surface-900">
        ...
        <template #end>
          <SplitButton
            icon="pi pi-plus"
            @click="() => uploadStore.browse(parentId)"
            :model="itemsAdd"
          />
        </template>
      </Menubar>
  ...
  <Dialog
    ...
    modal
    :header="$t('drive.create.folder.title')"
    :style="{ width: '50vw' }"
  >
      ...
      <Form id="folderForm" :resolver="resolver" @submit="onFormSubmit" class="w-full">
        <FormField v-slot="$field" name="name">
          ...
          <InputText
            id="folderName"
            size="large"
            :placeholder="$t('drive.create.folder.name')"
            class="w-full"
          />
    ...
    <template #footer>
      <Button
        :label="$t('drive.create.folder.cancel')"
        icon="pi pi-times"
        @click="showCreateFolderModal = false"
        class="p-button-text"
      />
      <Button
        :label="$t('drive.create.folder.submit')"
        icon="pi pi-check"
        type="submit"
        form="folderForm"
        autofocus
