<template>
  <Dialog :visible="visible" @update:visible="emit('update:visible', $event)" modal :header="$t('drive.file.move.title')" :style="{ width: '50vw' }">
    <div class="flex flex-col gap-4">
      <div class="flex items-center gap-2">
        <Button v-if="currentParentId" icon="pi pi-arrow-left" @click="goBack" class="p-button-text" />
    ...
    <template #footer>
      <Button :label="$t('drive.file.move.cancel')" icon="pi pi-times" @click="close" class="p-button-text" />
      <Button :label="$t('drive.file.move.submit')" icon="pi pi-check" @click="move" :disabled="!selectedFolder" autofocus />
    </template>
  </Dialog>
</template>
...
const move = async () => {
  ...
  try {
    const parentId = Object.keys(selectedFolder.value)[0] === 'root' ? 'root' : Object.keys(selectedFolder.value)[0]
